...
let fixed = 0;
const outputDirs = [join(process.cwd(), 'dist'), join(process.cwd(), '.vercel/output/static')];

for (const outputDir of outputDirs) {
  const astroDir = join(outputDir, '_astro');
  if (!existsSync(astroDir)) continue;

  const cssFiles = readdirSync(astroDir).filter((f) => /^ec\.[a-z0-9]+\.css$/.test(f));
...
console.log(
  fixed > 0
    ? `[fix-ec-css] 修正了 ${fixed} 处 EC CSS 引用`
